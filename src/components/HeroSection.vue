<template>
  <section id="hero" class="relative overflow-hidden" style="min-height: 540px;">
    ...
    <div class="relative z-10">
      <!-- Eyebrow -->
      <p class="font-mono text-[13px] tracking-[0.03em] m-0" style="color: #85837A;">
      ...
      >
        Stephanie<br>Mbusheri<span class="cursor-blink" aria-hidden="true">▍</span>
      </h1>

      <!-- Bio -->
      <p class="m-0 mt-7" style="color: #A8A69D; max-width: 580px; font-size: clamp(17px, 2.1vw, 20px); line-height: 1.55;">
        I build fast, trustworthy interfaces for products people use every day — payments, messaging, and commerce — working mostly in Vue, TypeScript, and Tailwind.
      </p>
      ...
      <!-- CTA row -->
      <div class="flex gap-[13px] mt-9 flex-wrap">
        <button
          class="font-mono text-[13px] cursor-pointer border-none font-medium tracking-[0.01em]"
          style="padding: 13px 22px; border-radius: 4px; background: var(--accent); color: #15170F; transition: filter .15s;"
          @click="emit('scroll-to', 'mesagoo')"
          @mouseenter="(e) => ((e.currentTarget as HTMLElement).style.filter = 'brightness(1.08)')"
        ...
        <button
          ...
          style="padding: 13px 22px; border-radius: 4px; background: transparent; border: 1px solid #34342d; transition: border-color .15s, background .15s;"
...
function onGhostEnter() {
  ...
  ghostBtn.value.style.background = 'rgba(255,255,255,0.02)'
}
